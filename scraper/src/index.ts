...
import { scrapeAccIntSkills, ACCINT_REPO } from './scrapers/accint.js';
...
async function main() {
  ...
  const stars: RepoStars = {
    ...
    accint: await fetchStars(ACCINT_REPO),
  };
  ...
  console.log(`  • AccInt: ${stars.accint.toLocaleString()} stars`);
  console.log('');
  ...
  try {
    const accintSkills = await scrapeAccIntSkills();
    allSkills.push(...accintSkills);
  } catch (err) {
    console.error('Error scraping AccInt Skills:', err);
  }
